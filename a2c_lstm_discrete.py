...
#------------------------------- neural network ------------------------------#
def lstm(x, masks, h, c, num_steps):
    inner_states = []
    for i in range(num_steps):
        h *= masks[i]
        c *= masks[i]
        h, c = PF.lstm_cell(x[i], h, c, 512, name='lstm')
        inner_states.append(h)
    return F.stack(*inner_states)


def cnn_network(obs, masks, h, c, num_actions, num_steps, scope):
    with nn.parameter_scope(scope):
        ...
        out = F.relu(out)
        out = F.reshape(out, (num_steps, -1, 512))
        out = lstm(out, masks, h, c, num_steps)
        out = F.reshape(out, (-1, 512))
...
class A2C:
    def __init__(self, num_actions, num_steps, num_envs, batch_size, v_coeff,
                 ent_coeff, lr_scheduler):
        self.h = nn.Variable((num_envs, 512))
        self.c = nn.Variable((num_envs, 512))

        self.infer_obs_t = nn.Variable((num_envs, 4, 84, 84))
        inference = cnn_network(
            self.infer_obs_t, F.constant(1.0, (1, num_envs, 1)), self.h,
            self.c, num_actions, 1, 'network')
        self.infer_pi_t, self.infer_value_t, self.hn, self.cn = inference
        self.infer_t = F.sink(*inference)
        ...
        self.masks_t = nn.Variable((num_steps, num_envs, 1))

        pi_t, value_t, _, _ = cnn_network(
            self.obs_t, self.masks_t, self.h, self.c, num_actions, num_steps,
            'network')
    ...
    def infer(self, obs_t, h, c):
        self.infer_obs_t.d = np.array(obs_t)
        self.h.d = np.array(h)
        self.c.d = np.array(c)
        self.infer_t.forward(clear_buffer=True)
        flatten_value = np.reshape(self.infer_value_t.d, (-1,))
        return self.infer_pi_t.d, flatten_value, self.hn.d, self.cn.d

    def train(self, obs_t, acts_t, rets_t, advs_t, masks_t, h, c, step):
        ...
        self.masks_t.d = np.expand_dims(np.array(masks_t), axis=-1)
        self.h.d = np.array(h)
        self.c.d = np.array(c)
        self.loss.forward()
...
def train_loop(env, model, num_actions, update_interval, return_fn, logdir):
    ...
    h, c = np.zeros((2, len(env.envs), 512), dtype=np.float32)
    ...
    while step <= 10 ** 7:
        ...
        # inference
        probs_t, value_t, h, c = model.infer(pixel_to_float(obs_t), h, c)
        # sample actions
        ...
        if len(obss_t) == update_interval:
            vals_t.append(value_t)
            rets_t = return_fn(vals_t, rewards_tp1, dones_tp1)
            advs_t = rets_t - vals_t[:-1]
            masks_t = np.vstack((np.zeros((1, len(env.envs))), dones_tp1[:-1]))
            policy_loss, value_loss = model.train(
                pixel_to_float(obss_t), acts_t, rets_t, advs_t,
                np.abs(masks_t - 1.0), init_h, init_c, step)
...
def main(args):
    ...
    model = A2C(num_actions, time_horizon, num_envs, num_envs * time_horizon,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', type=str, default='a2c_lstm')
