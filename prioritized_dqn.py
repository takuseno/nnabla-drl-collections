...
class PrioritizedDQN:
    def __init__(self, num_actions, batch_size, gamma, lr):
        ...
        self.weights = weights = nn.Variable((batch_size, 1))

        # training output
        q_t = cnn_network(obs_t, num_actions, scope='q_func')
        q_tp1 = cnn_network(obs_tp1, num_actions, scope='q_func')
        q_tp1_target = cnn_network(obs_tp1, num_actions, scope='target_q_func')

        # select one dimension
        a_t_one_hot = F.one_hot(actions_t, (num_actions,))
        ...
        y.need_grad = False
        self.td = F.huber_loss(q_t_selected, y)
        self.loss = F.sum(self.td * weights)
    ...
    def train(self, obs_t, actions_t, rewards_tp1, obs_tp1, dones_tp1, weights):
...
class Buffer:
    ...
    def sample(self):
        ...
        return indices, experiences, np.expand_dims(weights / weights.max(), -1)
...
def train(model, buffer):
    ...
    td, loss = model.train(pixel_to_float(obs_t), actions_t, rewards_tp1,
                           pixel_to_float(obs_tp1), dones_tp1, weights)
...
def main(args):
    ...
    model = PrioritizedDQN(num_actions, args.batch_size, args.gamma, args.lr)
    if args.load is not None:
        nn.load_parameters(args.load)

    # replay buffer for experience replay
    buffer = Buffer(args.buffer_size, args.batch_size, args.alpha, args.beta)
